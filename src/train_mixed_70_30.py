"""
Mixed 70/30 adversarial training script.
70% of training images are attacked with a randomly chosen attack type per bbox.
30% remain clean. All images get an augmented copy (~2x dataset size).

Usage:
    python src/train_mixed_70_30.py
"""
...
MODEL_IN  = os.path.join(ROOT, 'models', 'best.pt')
...
def build_expanded_train_set(base_path, train_folder, split_ratio=0.7):
    ...
    for i, img_name in enumerate(all_images):
        ...
        # Each bbox on attacked images gets a randomly chosen attack type
...
# Uses a random attack per bbox so the val set reflects all attack types
